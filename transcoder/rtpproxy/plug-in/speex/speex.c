...
BEGIN_EXPORTS("speex")
    ...
    BEGIN_CODECS
     CODEC( CODEC_SPEEX_NB, 1, Pcm16_2_SpeexNB, SpeexNB_2_Pcm16, speexNB_create, speexNB_destroy)
    END_CODECS
...
long speexNB_create(const char* format_parameters, amci_codec_fmt_info_t* format_description)
{
    ...
    */
    ...
    format_description[2].value = bits/8 + (bits % 8 ? 1 : 0);
    
    format_description[3].id = 0;
    
    DBG("SpeexState %ld inserted with mode %d and %d frames per packet,\n", (long)ss, ss->mode, ss->frames_per_packet);
		     ...
		     unsigned int channels, unsigned int rate, long h_codec )
{
    ...
    speex_bits_read_from(&decoder->bits, in_buf, size);
